--[[
    • ERC UI - library UI
    • library Version | global-erc-uilib-0.6.0.8
...
function erclib:window(config)
    local MAIN_TITLE = config.TitleText or "ERC UI - (setting default)"
    local DESC_TITLE = config.SubTitle or "By erc.t.tm.th | discord.gg/kShJy84u2v (setting default)"
    local MAIN_IMAGE = config.ImageIcon or "rbxassetid://138560507380517"
    local SIZE_MAIN = config.Size or UDim2.new(0,560,0,325)
...
function ContainerContent:createToggleDescription(config)
    ...
    local MAX_CONTAINER_Y = math.huge
      
    local MIN_LABELFRAME_Y = 28          
    local MAX_LABELFRAME_Y = math.huge
