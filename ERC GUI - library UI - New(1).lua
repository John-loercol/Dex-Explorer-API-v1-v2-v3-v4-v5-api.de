...
-- ฟังก์ชันสร้างแถบโหลด (percent = 0-1)
local function progressBar(percent, barLength)
    local filled = math.floor(percent * barLength)
    local bar = string.rep("#", filled) .. string.rep("-", barLength - filled)
    return string.format("[%s] %3d%%", bar, percent * 100)
end

-- รายการ assets
local assets = {
    "Initializing core", "Loading fonts", "Loading icons", 
    "Loading shaders", "Loading textures", "Loading sounds",
    "Loading animations", "Loading layouts", "Loading components",
    "Loading themes", "Compiling scripts", "Applying styles",
    "Mounting widgets", "Finalizing"
}
...
for _, assetName in ipairs(assets) do
    local percent = 0
    warn(string.format("[ ⚠ ] Loading :: %s", assetName))
    
    -- สุ่มเปอร์เซ็นต์เพิ่มทีละ 1-20 จนถึง 100%
    while percent < 100 do
        local increment = math.random(1, 20)
        percent = math.min(percent + increment, 100)
        
        local bar = progressBar(percent / 100, barLength)
        warn(string.format("%s - %s", bar, assetName))
        
        -- สุ่มดีเลย์ระหว่าง 0.1 ถึง 0.6 วินาที (ทำให้รวมเวลาทั้ง asset อยู่ในช่วง 0.5-10 วิ โดยประมาณ)
        local delay = math.random() * 2.5 + 0.1  -- 0.1 - 0.6 วินาที
        task.wait(delay)
    end
    
    warn(string.format("[ ✓ ] Load :: %s successfully!", assetName))
    warn("-----------------------------------")
end

warn("[ ✓ ] Loading all accets files is complete.")
     end)
task.wait(5)
...
return {
    ...
    - api:GetColor() :: ขอค่าสี
    - api:SetColor() :: ตั้งค่าสี
