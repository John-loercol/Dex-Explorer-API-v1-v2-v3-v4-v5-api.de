...
--[[
    • ERC GUI - library UI
    • Version | V.0.6.0.0
    • Developed by erc.t.tm.th / ERC-Technology™
    • News channels | https://discord.gg/kShJy84u2v
    • Optimized for production – Delta Executor (Android)
]]
...
--// Constants
--[[
Delta New :: 105145135555552
Delta Old :: 13501536441
Krnl      :: 11671355800
ERC       :: 138560507380517
]]

local MAIN_IMAGE = "rbxassetid://105145135555552"

local NOTIFICATION_SOUND_ID = "rbxassetid://87437544236708"

local MAIN_TITLE = "ERC GUI - V.0.6.0.0 - Developer Console (Beta)"

local DESC_TITLE = "By erc.t.tm.th | discord.gg/::::::::"

local ICON_TEXT = "ERC GUI"

local NOTIF_TITLE = "ERC SYSTEM"

local FontGUI = Enum.Font.SourceSansBold
...
local function Notification(config)
...
createTextSizeConstraint(MainIconText,14,20)

local iconDiv = Instance.new("Frame")
iconDiv.Size = UDim2.new(0,2,1,0)
iconDiv.Position = UDim2.new(0,100,0,0)
...
createTextSizeConstraint(titleLabel,16,18)
...
createTextSizeConstraint(descLabel,14,16)
...
local function getCenterPosition(frame)
    ...
    if frame == MainFrame then
        return UDim2.new(0, (vs.X - frame.AbsoluteSize.X)/2, 0, (vs.Y - frame.AbsoluteSize.Y)/100)
    elseif frame == MainIcon then
        return UDim2.new(0, (vs.X - frame.AbsoluteSize.X)/2, 0, (vs.Y - frame.AbsoluteSize.Y)/100)
    end
end
...
minimizeBtn.MouseButton1Click:Connect(function()
    TweenMainFrameOut(function() TweenIconIn() end)
end)
...
fullscreenBtn.MouseButton1Click:Connect(function()
    Notification({ MessageText = "[ ⚠ ] Coming soon in the next update.", DelayTime = 2 })
end)
    ...
    container.Position = UDim2.new(0,280,0,162.5)
...
pageFrame.Size = UDim2.new(1,-180,1,-37.8)
pageFrame.Position = UDim2.new(0,180,0,37.8)
pageFrame.BackgroundColor3 = Color3.fromRGB(30,30,30)
pageFrame.BackgroundTransparency = 0
pageFrame.ZIndex = 19999
pageFrame.ClipsDescendants = true -- กันล้นขอบ
...
local function CreateTab(config)
    ...
    createUICorner(page)
    createListLayout(page,8,6,2.5,0)
...
local function protectInstance(instance, propertyMap)
    if not instance then return end

    for property, desiredValue in pairs(propertyMap) do
        ...
        local conn = instance:GetPropertyChangedSignal(property):Connect(function()
            ...
            if instance[property] ~= desiredValue then
                isProtecting = true
                instance[property] = desiredValue
                isProtecting = false
            end
        end)

        table.insert(protectionConnections, conn)
...
-- เริ่มต้นป้องกันค่าต่างๆ
protectInstance(ERCFrame, {
    ZIndex = 2147483647
})

protectInstance(MainTitleimg, {
    Image = MAIN_IMAGE
})

protectInstance(MaintitleLabel, {
    Text = MAIN_TITLE
})

protectInstance(MainiconText, {
    Text = ICON_TEXT
})

protectInstance(DescriptiontitleLabel, {
    Text = DESC_TITLE
})

protectInstance(NotificationSound, {
    SoundId = NOTIFICATION_SOUND_ID
})

-- เมื่อ GUI ถูกทำลาย ให้ตัดการเชื่อมต่อทั้งหมด
...
ERCFrame.Destroying:Connect(cleanup)

-- </Protection System >

-- <Function createToggle>
--// Function: ฟังก์ชัน สร้างแผงสำหรับหัวข้อกลุ่ม
local function createToggleSection(config)
    -- config
...
local function createToggleDescription(config)
    -- config
    local Parent = config.Parent
    local Name   = config.Description or "Description"

    local MIN_CONTAINER_Y = 35          
    local MAX_CONTAINER_Y = math.huge -- 7800
      
    local MIN_LABELFRAME_Y = 28          
    local MAX_LABELFRAME_Y = math.huge -- 7100
...
local function createToggleText(config)
    -- config
    local Parent = config.Parent
...
local function createToggleTextLabel(config)
    -- config
    local Parent = config.Parent
...
local function createToggleSlider(config)
    -- config
    local Parent = config.Parent
    local Name = config.Name or ""
...
-- =================================
local function createToggleDropdown(config)
    -- config
    local Parent = config.Parent
    local Name = config.Name or ""
    local Description = config.Description or ""
    ...
    containerDropdown.Position = UDim2.new(0,280,0,162.5)
    ...
    -- Dropdown API
    -- =================================
    local Dropdown = {}  -- เก็บ API functions
    local optionButtons = {}
    
    -- สถานะการเลือกภายใน
    Dropdown._selection = nil
    Dropdown._multiSelection = {}
    Dropdown._mode = mode
    ...
    local function _restoreSelection()
        if Dropdown._mode == "Single" and Dropdown._selection then
            -- ค้นหาปุ่มที่ตรงกับค่าที่เลือกไว้
            local buttonData = optionButtons[Dropdown._selection]
            if buttonData then
                TweenService:Create(buttonData.Stroke, TweenInfo.new(0.15), {
                    Color = Color3.fromRGB(0,150,255)
                }):Play()
                buttonData.IsSelected = true
                inputBox.Text = Dropdown._selection
            else
                -- ถ้าค่าที่เลือกไว้ไม่มีในรายการปัจจุบัน
                inputBox.Text = DefaultText
            end
        elseif Dropdown._mode == "Multi" then
            local selectedNames = {}
            for value, selected in pairs(Dropdown._multiSelection) do
                if selected then
    ...
    local function createOptionButton(text)
        ...
        if Dropdown._mode == "Single" and Dropdown._selection == text then
            buttonData.IsSelected = true
            TweenService:Create(stroke, TweenInfo.new(0.15), {
                Color = Color3.fromRGB(0,150,255)
            }):Play()
        elseif Dropdown._mode == "Multi" and Dropdown._multiSelection[text] then
            buttonData.IsSelected = true
            TweenService:Create(stroke, TweenInfo.new(0.15), {
                Color = Color3.fromRGB(0,150,255)
            }):Play()
        ...
        button.MouseButton1Click:Connect(function()
            if Dropdown._mode == "Single" then
                -- ถ้าปุ่มนี้ถูกเลือกอยู่แล้ว (กดซ้ำ) ให้ยกเลิกการเลือก
                if Dropdown._selection == text then
                    -- ยกเลิกการเลือก
                    TweenService:Create(stroke, TweenInfo.new(0.15), {
                        Color = Color3.fromRGB(45,45,45)
                    }):Play()
                    ...
                    -- reset ปุ่มเก่า
                    if Dropdown._selection then
                        local oldButtonData = optionButtons[Dropdown._selection]
            ...
            elseif Dropdown._mode == "Multi" then
                -- ใช้ value เป็น key
                if Dropdown._multiSelection[text] then
                    -- ยกเลิกเลือก (กดซ้ำ)
                    Dropdown._multiSelection[text] = nil
                    buttonData.IsSelected = false
                    TweenService:Create(stroke, TweenInfo.new(0.15), {
                        Color = Color3.fromRGB(45,45,45)
                    }):Play()
                    ...
                    -- เลือก
                    Dropdown._multiSelection[text] = true
                    buttonData.IsSelected = true
                    TweenService:Create(stroke, TweenInfo.new(0.15), {
                        Color = Color3.fromRGB(0,150,255)
                    }):Play()
    ...
    -- เพิ่มหลายปุ่ม
    function Dropdown:AddList(list)
        -- เก็บค่าที่เลือกไว้ก่อนเพิ่มปุ่มใหม่
        local oldSelection = Dropdown._selection
        local oldMultiSelection = {}
        for k, v in pairs(Dropdown._multiSelection) do
    ...
    function Dropdown:Set(value)
        if Dropdown._mode == "Single" then
            ...
            if Dropdown._selection == value then
                local buttonData = optionButtons[value]
                if buttonData then
                    TweenService:Create(buttonData.Stroke, TweenInfo.new(0.15), {
                        Color = Color3.fromRGB(45,45,45)
        ...
        elseif Dropdown._mode == "Multi" then
            local buttonData = optionButtons[value]
            if buttonData then
                if Dropdown._multiSelection[value] then
                    -- ถ้ามีอยู่แล้ว (กดซ้ำ) ให้ยกเลิก
                    Dropdown._multiSelection[value] = nil
                    buttonData.IsSelected = false
                    TweenService:Create(buttonData.Stroke, TweenInfo.new(0.15), {
                        Color = Color3.fromRGB(45,45,45)
                    }):Play()
                    ...
                    -- เลือกใหม่
                    Dropdown._multiSelection[value] = true
                    buttonData.IsSelected = true
                    TweenService:Create(buttonData.Stroke, TweenInfo.new(0.15), {
                        Color = Color3.fromRGB(0,150,255)
                    }):Play()
                ...
                -- update display text
                local names = {}
                for val, selected in pairs(Dropdown._multiSelection) do
                    if selected then
    ...
    function Dropdown:SetMultiple(values)
        if Dropdown._mode ~= "Multi" then
    ...
    -- ตั้งโหมด
    function Dropdown:SetMode(mode)
...
-- ฟังก์ชันรีเฟรชสถานะปุ่มทั้งหมด (ใช้เมื่อปิด/เปิด dropdown)
...
local function createToggleColorPicker(config)
    -- config
    local Parent = config.Parent
    local Name = config.Name or ""
    local Description = config.Description or ""
    ...
    statusLight.Size = UDim2.new(0.45, -90, 0.8, 0)
    statusLight.Position = UDim2.new(0.55, 85, 0.1, 0)
    ...
    containerRGB.Position = UDim2.new(0,280,0,162.5)
...
--// script ที่ใช้งานภายในไฟล์นี้ที่ไม่ได้โดนเรียกจากภายนอกเป็นแค่การเทสระบบเฉยๆเรายังไม่ได้ทำตัวเรียกใช้จากภายนอก เหมือนตัว library อื่นๆ //--
...
-- <Pages All>
-- Page: Main
local MainPage = CreateTab({NamePage = "Main"})

createToggleDescription({
    Parent = MainPage,
    Description = "[ information ]\n• ERC GUI - Developer Console (Beta)\n• Version | V.0.6.0.0\n• Developed by erc.t.tm.th / K-9 \n• News channels | discord.gg/::::::::\n• I'm working with the Lua language in Roblox.\n[ Update ]\n• This update is a completely new UI update."
})
...
createToggleButton({
	...
	Text = "Button",
	callback = function()
		CopyText("https://discord.gg/kShJy84u2v")
		Notification({
			TitleText = "Link Discord",
...
-- ===================================================
-- Page: Player
local PlayerPage = CreateTab({NamePage = "Player"})
...
local getFly = createToggle({
    Parent = PlayerPage,
    Name = "FLY GUI",
    Description = "Click to open GUI"
})

local getInfiniteJump = createToggle({
    Parent = PlayerPage,
    Name = "Infinite Jump",
    Description = "Click to open Infinite Jump"
})

local getNoClip = createToggle({
    Parent = PlayerPage,
    Name = "NoClip",
    Description = "Click to open NoClip"
})

local getShiftLock = createToggle({
    Parent = PlayerPage,
    Name = "Shift Lock",
    Description = "Click to open Shift Lock"
})

-- ===================================================  
-- Page: Visual
local VisualPage = CreateTab({NamePage = "Visual"})
...
local getRemovefog = createToggle({
    Parent = VisualPage,
    Name = "Remove fog",
    Description = "Click to open Remove fog"
})

local getGrayscaleMode = createToggle({
    Parent = VisualPage,
    Name = "Grayscale Mode",
    Description = "Click to open Grayscale Mode"
})

local getVividMode = createToggle({
    Parent = VisualPage,
    Name = "Vivid Mode",
    Description = "Click to open Vivid Mode"
})

local getESPPlayer = createToggle({
    Parent = VisualPage,
    Name = "ESP Player",
    Description = "Click to open ESP Player"
})

local getXray = createToggle({
    Parent = VisualPage,
    Name = "X-ray",
    Description = "Click to open X-ray"
})

local getFPS = createToggle({
    Parent = VisualPage,
    Name = "FPS GUI",
    Description = "Click to open GUI"
})

local getInfiniteZoom = createToggle({
    Parent = VisualPage,
    Name = "Infinite Zoom",
    Description = "Click to open Infinite Zoom"
})

local getblackscreen = createToggle({
    Parent = VisualPage,
    Name = "Black Screen",
    Description = "Click to open Black Screen"
})

local getWhitescreen = createToggle({
    Parent = VisualPage,
    Name = "white Screen",
    Description = "Click to open Black Screen"
})
...
-- ===================================================  
-- Page: Function
local FunctionPage = CreateTab({NamePage = "Tools"})
...
createToggleButton({
    Parent = FunctionPage,
    Name = "[ ❖ ] Roblox Developer Console",
    Description = "Click to Open the Developer Console",
    Text = "Button",
    callback = function()
        Notification({MessageText = "Roblox Developer Console Loading!", DelayTime = 5})
        DevConsole()
    end
})
...
createToggleButton({
    Parent = FunctionPage,
    Name = "[ ❖ ] Rejoin",
    Description = "Rejoins your current server",
    Text = "Button",
    callback = function()
...
createToggleButton({
    Parent = FunctionPage,
    Name = "[ ❖ ] Small Server",
    Description = "Joins a server with a low playercount",
    Text = "Button",
    callback = function()
...
createToggleButton({
    ...
    Description = "Click to run the script",
    Text = "Button",
    callback = function()
    Notification({MessageText = "Script Infinite Yield FE Loading!", DelayTime = 5})
...
createToggleButton({
    ...
    Text = "Button",
    callback = function()
        Notification({MessageText = "Script Dex Explorer Loading!", DelayTime = 5})
...
createToggleButton({
    ...
    Text = "Button",
    callback = function()
        Notification({MessageText = "Script Simple Shader Loading!", DelayTime = 5})
...
createToggleButton({
    ...
    Text = "Button",
    callback = function()
        Notification({MessageText = "Script Disconnection Loading!", DelayTime = 5})
        local function showDisconnect(reason)  
            pcall(function()  
                player:Kick(reason or "ERROR 404")
            end)  
        end  
        showDisconnect("[ ❖ ] ERC SYSTEM | Disconnection Successfully!")
    end
})

-- ===================================================  
-- Page: Teleport
local TeleportPage = CreateTab({NamePage = "Teleport"})

local PlayersAPI = createToggleDropdown({
    Parent = TeleportPage,
    Name = "Target Player",
    mode = "Single",
...
local TeleportLooped = createToggle({
    Parent = TeleportPage,
    Name = "Follow Teleport",
    Description = "Continuously teleport to target"
})

local TeleportClick = createToggle({
    Parent = TeleportPage,
    Name = "Click Teleport",
    Description = "Tap anywhere to teleport"
})
...
local TeleportOne = createToggleButton({
    Parent = TeleportPage,
    Name = "Teleport Once",
    Description = "Instant teleport to target player",
    Text = "Button",
    callback = function()
        local selected = PlayersAPI:Get()
        if type(selected) == "string" then
            teleportToPlayerByName(selected)
        end
    end
})

RunService.Heartbeat:Connect(function()
    if TeleportLooped:GetState() then
        local selected = PlayersAPI:Get()
        if type(selected) == "string" then
            teleportToPlayerByName(selected)
        end
...
local function refreshPlayerDropdown()
    ...
    -- เพิ่มตัวเลือกผู้เล่นทั้งหมด (ยกเว้นตัวเรา)
...
-- ==========================
-- Teleport Click Logic
-- ==========================
-- ระบบ Teleport แบบคลิก
local Player = game.Players.LocalPlayer
local Mouse = Player:GetMouse()
local TweenService = game:GetService("TweenService")

-- ตัวแปรเก็บ connection เพื่อป้องกัน memory leak
local teleportConnection = nil

-- ฟังก์ชัน teleport ไปยังตำแหน่งที่คลิก
local function teleportToClick()
    ...
    -- ใช้ Tween เพื่อให้การเคลื่อนไหวลื่นไหล
    local tweenInfo = TweenInfo.new(
        0.2, -- เวลา
        Enum.EasingStyle.Quad, -- รูปแบบ easing
...
local function updateTeleportState()
    ...
    if TeleportClick:GetState() then
        teleportConnection = Mouse.Button1Down:Connect(function()
            if not game:GetService("UserInputService"):GetMouseLocation() then return end
            teleportToClick()
        end)
    end
end
...
-- ===================================================
-- Page: Music
local musicPage = CreateTab({NamePage = "Music"})
...
-- UI
-- ==========================
local SongName = createToggleSection({
    Parent = musicPage,
    Name = "Song : None"
})

local Status = createToggleSection({
    Parent = musicPage,
    Name = "Status : System is on standby."
})

local TimeSong = createToggleTextLabel({
    Parent = musicPage,
    Name = "Time Song",
...
local idBox = createToggleText({
    Parent = musicPage,
    Name = "ID Song",
    Description = "Click to Message",
    Text = "108531350726198",
    PlaceholderText = "ID..."
})
...
-- ==========================
-- Music Speed Slider (Decimal 1x → 10x)
-- ==========================
local currentSpeed = 1
local Musicspeed = createToggleSlider({
    Parent = musicPage,
    Name = "Song speed",
    mode = "decimal",
    default = 1,
...
-- Looped
-- ==========================
local Looped = createToggle({
    Parent = musicPage,
    Name = "Looped",
    Description = "Click to open Looped"
})
...
RunService.RenderStepped:Connect(function()
    ...
    if sound.SoundId ~= "" and sound.TimeLength > 0 then
        TimeSong:SetText(formatTime(sound.TimePosition) .. " | " .. formatTime(sound.TimeLength))
    else
        TimeSong:SetText("00:00 : 00:00")
    end

    -- Loop
    sound.Looped = Looped:GetState()
...
local PlaySong = createToggleButton({
    ...
    callback = function()
        ...
        if not ok or not info or info.AssetTypeId ~= 3 then
            SongName:SetText("Please use only the song ID.")
            setStatus("Status : Restricted / Not an audio asset.")
            return
        end
        ...
        if sound.SoundId ~= assetId then
            ...
            isLoading = false
            sound:Play()
            Notification({MessageText = "Song is playing.", DelayTime = 2})
        else
...
local StopSong = createToggleButton({
    ...
    callback = function()
        if sound.IsPlaying then
            sound:Pause()
            Notification({MessageText = "Stop the Song.", DelayTime = 2})
        end
    end
})
...
local Reset = createToggleButton({
    ...
    callback = function()
        if sound.SoundId ~= "" then
            ...
            sound.PlaybackSpeed = currentSpeed
            sound:Play()
            Notification({MessageText = "Song reset complete.", DelayTime = 2})
        end
    end
})
...
local CopyID = createToggleButton({
    ...
    Text = "Button",
    callback = function()
        if idBox:GetText() ~= "" then
            CopyText(idBox:GetText())
            Notification({MessageText = "Copying is complete.", DelayTime = 2})
        end
    end
})
...
-- ===================================================  
-- Page: Sigma Spy
local SigmaSpy = CreateTab({NamePage = "Sigma Spy"})

--[[
createToggleDescription
• API ที่เชื่อมต่อกับปุ่ม  
- api:SetText("...") สามารถเขียนข้อความลงไปใหม่ได้
- api:ClearText() สามารถลบข้อความที่เขียนได้

createToggleDropdown
• API ที่เชื่อมต่อกับปุ่ม  
- api:Add("...") สามารถเพิ่มตัวเลือก
- api:Get() ใช้รับค่าตัวเลือก
- api:Clear() ใช้ลบตัวเลือกทั้งหมด
]]

-- ===================================================  
-- Page: Chatbot
local Chatbot = CreateTab({NamePage = "AI Chat"})
...
-- Output panel
-- =========================
local OutputMessage = createToggleDescription({
    Parent = Chatbot,
    Description =
"[ information ]\n"..
"• AI Chat ready.\n\n"..
...
-- =========================
-- Chat Log + Memory System
-- =========================

local MAX_MEMORY_BYTES = 50 * 1024 * 1024
...
local function memorySize()
    local ok, encoded = pcall(function()
        return HttpService:JSONEncode(chatMemory)
    end)
    return ok and #encoded or 0
end

local function trimMemory()
    while memorySize() > MAX_MEMORY_BYTES and #chatMemory > 1 do
        table.remove(chatMemory, 2)
    end
end

local function addMemory(role, content)
    table.insert(chatMemory, {
        role = role,
...
local function resetMemoryOnly()
    chatMemory = {
        {role = "system", content = "You are a helpful assistant."}
    }
    ...
    Notification({
        TitleText="AI",
        MessageText="Memory cleared (chat kept)",
        DelayTime=2
    })
end

local function appendChat(userText, aiText)

    messageIndex += 1

    local memMB =
        math.floor(memorySize()/1024/1024*100)/100

    local entry =
"— • ================================================= • —\n"..
"[ ✓ ] Replying to message #" .. messageIndex .. "\n\n"..
"[ information ]\n"..
"• Memory used: "..memMB.." MB / 50 MB\n\n"..
"[ Input Message ]\n~ $ "..userText.."\n\n"..
"[ Output Message ]\n~ $ "..aiText.."\n\n"

    chatLogText = chatLogText .. entry

    OutputMessage:SetText(chatLogText)
    lastOutputText = chatLogText
end

-- =========================
-- Model dropdown
-- =========================
local ChatbotMode = createToggleDropdown({
    Parent = Chatbot,
    Name = "AI Model",
    mode = "Single",
    Description = "Select AI model.",
    DefaultText = "Initial model — Chat GPT"
})

    ChatbotMode:Add("Chat GPT — Open AI")

-- =========================
-- Input box
-- =========================
local InputMessage = createToggleText({
    Parent = Chatbot,
    Name = "Message",
...
local function startLoading(userText)
...
"- $ "..loadingFrames[i]
            ...
            i += 1
            if i > #loadingFrames then i = 1 end
...
createToggleButton({
    Parent = Chatbot,
    Name = "Send Message",
    Description = "Send to AI",
    Text = "Send",

    callback = function()

        local userMessage = InputMessage:GetText()
        local now = os.clock()

        if userMessage == "" then
            Notification({TitleText="AI",MessageText="Enter message",DelayTime=2})
            return
        end

        if now - lastSendTime < cooldown then
            Notification({TitleText="AI",MessageText="Please wait",DelayTime=2})
            return
        end

        lastSendTime = now

        startLoading(userMessage)
        ...
        task.spawn(function()
            ...
            if success then
                ...
                addMemory("assistant", result)
                lastAIReply = result

                appendChat(userMessage, result)

                Notification({
                    TitleText="AI",
                    MessageText="Response received",
                    DelayTime=1.5
                })
...
-- Voice playback (TTS)
-- =========================
local function playVoice(text)
    if text == "" then
        Notification({TitleText="Voice", MessageText="No message", DelayTime=2})
        return
    end

    local encoded = HttpService:UrlEncode(text)
    -- ใช้ API ของ Streamelements (ไม่ต้องใช้ key)
    local url = "https://api.streamelements.com/kappa/v2/speech?voice=Amy&text="..encoded

    local played = false

    -- ตรวจสอบและเรียกใช้ฟังก์ชันของ executor
    if syn and type(syn.play_audio) == "function" then
        syn.play_audio(url)
        played = true
    elseif type(playAudio) == "function" then
        playAudio(url)
        played = true
    elseif type(play_sound) == "function" then  -- เผื่อ executor ใช้ชื่ออื่น
        play_sound(url)
        played = true
    end
    ...
    if played then
        Notification({
            TitleText = "Voice",
            MessageText = "Playing voice...",
            DelayTime = 1.5
        })
    else
        Notification({
            TitleText = "Voice",
            MessageText = "Your executor does not support audio playback.",
            DelayTime = 3
        })
    end
end

-- ปุ่ม Speak AI (แก้ไขแล้ว ไม่มี CopyText ที่ผิด)
createToggleButton({
    Parent = Chatbot,
    Name = "Speak AI",
    ...
    Text = "Speak",
    callback = function()
        playVoice(lastAIReply)   -- ใช้ข้อความล่าสุดจาก AI
    end
})
...
createToggleButton({
    ...
    callback = function()
        ...
        if chatLogText == "" then
            Notification({TitleText="Copy",MessageText="Nothing to copy",DelayTime=2})
            return
        end

        CopyText(chatLogText)

        Notification({TitleText="Copy",MessageText="Copied",DelayTime=1.5})
    end
})
