...
function erclib:Notification(config)
    ...
    img.Image = "rbxassetid://105145135555552"
...
function erclib:window(config)
    ...
    local ICON_TEXT = config.IconText or "ERC GUI"
...
-- ออกจาก Fullscreen
-- เต็มจอ → กลางจอ (ขนาดเดิม) → ตำแหน่งเดิมที่บันทึกไว้
local function exitFullscreen(callback)
    ...
    -- Step 1: ย่อขนาดลงมาอยู่กลางจอแบบไม่เด้ง
    smoothTween(MainFrame, 0.18, {
        Size = restoreSize,
        Position = centerPos
    }, Enum.EasingStyle.Quad, function()

        -- Step 2: เลื่อนไปตำแหน่งเดิมแบบ smooth
...
fullscreenBtn.MouseButton1Click:Connect(function()
    ...
    if isFullscreen then
        ...
        smoothTween(MainFrame, 0.18, {
            ...
            -- Step 2: ขยายแบบไม่มีเด้ง + เว้นซ้าย/ขวา/ล่าง
            local vs = Camera.ViewportSize
...
-- อัปเดต fullscreen เมื่อขนาดหน้าจอเปลี่ยน
