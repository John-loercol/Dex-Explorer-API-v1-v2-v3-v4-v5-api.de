...
local function GetGui()
    ...
    local parent =
        RunService:IsStudio() and LocalPlayer:FindFirstChild("PlayerGui")
        or (type(gethui) == "function" and gethui())
        or (cloneref and cloneref(CoreGui) or CoreGui)
...
ERCFrame.Name = "DevConsoleLayout"
...
ERCGui.Destroying:Connect(function()
    destroyed = true
end)

--// Auto Restore (กันโดนลบ)
task.spawn(function()
    while not destroyed do
        task.wait(0.5)

        if not ERCGui.Parent then
            local newParent =
                RunService:IsStudio() and LocalPlayer:FindFirstChild("PlayerGui")
                or (type(gethui) == "function" and gethui())
                or (cloneref and cloneref(CoreGui) or CoreGui)

            ERCGui.Parent = newParent
        end
    end
end)
...
--// Config Notification setting default
...
function erclib:window(config)
    local MAIN_TITLE = config.TitleText or "ERC GUI - (setting default)"
    local DESC_TITLE = config.SubTitle or "By erc.t.tm.th | discord.gg/:::::::: (setting default)"
    local MAIN_IMAGE = config.ImageIcon or "rbxassetid://105145135555552"
    local ICON_TEXT = config.IconText ro "ERC GUI"
...
local function switchPage(pageName)
    if not Tabs[pageName] then return end
    if currentPageName == pageName then return end

    switchToken += 1
    local myToken = switchToken

    local newPage = Tabs[pageName]
    local oldPage = currentPageName and Tabs[currentPageName]
...
function createToggleDescription(config)
    -- config
    local Parent = config.Parent
...
function createToggleText(config)
    -- config
    local Parent = config.Parent
...
function createToggleTextLabel(config)
    -- config
    local Parent = config.Parent
...
function createToggleSlider(config)
    -- config
    local Parent = config.Parent
    local Name = config.Name or ""
...
function createToggleDropdown(config)
    -- config
    local Parent = config.Parent
    local Name = config.Name or ""
    local Description = config.Description or ""
